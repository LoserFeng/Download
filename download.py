...
src_url="https://plus.figshare.com/ndownloader/files/39472855"
...
class Downloader():
    ...
    def load_dl_progress(self):
        ...
        if os.path.exists(self.dl_progress_path):
            with open(self.dl_progress_path, 'r') as f:
                self.dl_progress = json.load(f)
    ...
    def download_by_stream(self,resume=True):
        headers = {}
        
        if os.path.exists(self.dl_progress_path) and resume:
            ...
            print(f' start download from beginning  {self.dl_progress}')
...
def main():
    ...
    downloader.download_by_stream(False)
